...
export default class Player extends React.Component {
    ...
    componentDidMount() {
        ...
        Mousetrap.bind('ctrl+enter', () => {
            ...
            if (current) {
                this.setState({selectedLine: current}, () => window.$('#line-modal').modal('toggle'));
            }
        })
    ...
    getPreviousSubtitle = () => {
        ...
        const currentMS = currentTime * 1000;
        let list = this.state.subtitles || [];
        let currentIndex = list.findIndex(it => it.end > currentMS && it.start < currentMS);
        ''
    ...
    render() {
        ...
        const {name: title, image: {url: imageUrl}, sound: {url: soundUrl},} = lesson;
        const {silenceDuration, currentTime, subtitles: subs,} = this.state;
        const {autoScroll, showSettings, skipSilence} = this.state;
            ...
            <>
                ...
                <div className={'my-container ' + (showSettings ? 'showSettings' : '')}>
                    <div className={'top_container'}>
                        ...
                        <div id="scroll-div" className={'container ' + 'scroll_section' + ' ' + 'textContainer'}>
                            <h2 className={'noselect pb-3 mb-3'}>{title}</h2>

                            <SubtitleList
                                skipSilence={skipSilence}
                                onItemClick={(item) => this.player.currentTime = item.start / 1000}
                                onItemDoubleClick={(item) => {
                                    this.setState({selectedLine: item}, () => window.$('#line-modal').modal('show'))
                                }}
                                currentMs={currentMs} subtitles={subtitles}
                            ...
                            <div className="row text-light w-100 mx-5">
                                <div className="col-12 col-sm-6">
                                    ...
                                    <div className="w-100 h-100">
                                        <a href={"/lessons/" + previous_lesson_id} className="w-100 link text-light">
                                            <div className="prev-btn justify-content-center">
                                                <img src="/icons/skip_previous-white-18dp.svg" alt=""/>
                                                <p>
                                                    Previous
                                                </p>
                                            </div>
                                ...
                                <div className="col-12 col-sm-6">
                                    {next_lesson_id &&
                                    <div className=" w-100 h-100">
                                        <a href={"/lessons/" + next_lesson_id} className="w-100 link text-light">
                        ...
                        <div className={"icon-toggle-settings " + (showSettings ? 'active' : '')}
                    ...
                    <div className="controls h-6 d-flex px-3 bg-white align-items-center">
                        ...
                        >
                            <div
                                ...
                                className={"btn " + (autoScroll ? 'btn-outline-success' : 'btn-outline-secondary')}>Auto
                                Scroll
                            </div>
                        </div>
                        ...
                        >
                            <div
                                className={"btn " + (skipSilence ? 'btn-outline-success' : 'btn-outline-secondary')}>Skip
                                Silence
                            </div>
                        </div>
                        {skipSilence &&
                        <div className="d-flex align-items-center ml-3 pointer ">
                            ...
                            <input type="number" value={silenceDuration}
                                   style={{maxWidth: 90}}
...
function is_scrolling() {
    return window.lastScrollTime && new Date().getTime() < window.lastScrollTime + 999
}
